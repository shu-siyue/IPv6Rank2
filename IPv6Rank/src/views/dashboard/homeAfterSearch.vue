<template>
  <div style="display: flex; flex-direction: column; min-height: 100vh;">
    <div class="not-bottom">
      ...
      <div class="content">
        <template v-if="this.input_type === 'ipv6'">
          <map-dot :input="search" v-if="isSelected === 0" />
          <topo-v6 :input="search" v-if="isSelected === 1" />
          <path-graph :input="search" v-if="isSelected === 2" />
          <probenum-graph :input="search" v-if="isSelected === 3" />
          <asnum-graph :input="search" v-if="isSelected === 4" />
          <relation-graph :input="search" v-if="isSelected === 5" />
          <whois :input="search" v-if="isSelected === 6" />
          <website-domain :input="search" v-if="isSelected === 7"/>
        </template>
        <template v-else-if="this.input_type === 'AS'">
        </template>
        <template v-else-if="this.input_type === 'country'">
        </template>
        <template v-else>
          <div class="wrong">请输入正确的v6地址/AS号/国家名称</div>
        </template>
      </div>
    </div>
    <div class="bottom">
